...
OutputEquityScale ( const float r, const cubeinfo *pci, 
  ...
  if ( ! pci->nMatchTo ) {
    ...
    if ( fOutputMWC ) {
      ...
      if ( fOutputMatchPC ) {
        sprintf ( sz, "%*.*f%%",fOutputDigits + 3, fOutputDigits > 1 ? fOutputDigits - 1 : 0, 100.0f * ( f ? eq2mwc ( r, pci ) : se_eq2mwc ( r, pci ) ) );
      }
      else {
        sprintf( sz, "%*.*f", fOutputDigits + 3, fOutputDigits + 1, f ? eq2mwc ( r, pci ) : se_eq2mwc ( r, pci ) );
      }
...
static int DumpRace(const TanBoard UNUSED(anBoard), char *UNUSED(szOutput),
		    const bgvariation UNUSED(bgv))
{
